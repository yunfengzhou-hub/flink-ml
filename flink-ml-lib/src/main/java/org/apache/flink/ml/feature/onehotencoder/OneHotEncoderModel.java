...
public class OneHotEncoderModel implements Model<OneHotEncoderModel>, OneHotEncoderModelParams<OneHotEncoderModel> {
    private final Map<Param<?>, Object> paramMap = new HashMap<>();
    private static final String broadcastModelKey = "OneHotModelStream";
    ...
    public Table[] transform(Table... inputs) {
        ...
        RowTypeInfo outputTypeInfo =
                new RowTypeInfo(
                        ArrayUtils.addAll(inputTypeInfo.getFieldTypes(), Collections.nCopies(outputCols.length, ExternalTypeInfo.of(Vector.class)).toArray(new TypeInformation[0])),
        ...
        Function<List<DataStream<?>>, DataStream<Row>> function =
                dataStreams -> {
                    ...
                    return stream.transform(
                            ...
                            outputTypeInfo,
                            new OneHotPredictOp(new OneHotPredictFunc(inputCols, dropLast)));
                };
    ...
    private static class OneHotPredictOp
        extends AbstractUdfStreamOperator<Row, OneHotPredictFunc>
        implements OneInputStreamOperator<Row, Row> {

        public OneHotPredictOp(OneHotPredictFunc userFunction) {
            super(userFunction);
        }
    ...
    private static class OneHotPredictFunc extends RichMapFunction<Row, Row> {
        private final String[] inputCols;
        private final boolean dropLast;

        private OneHotPredictFunc(String[] inputCols, boolean dropLast) {
        ...
        public Row map(Row row) {
            List<Tuple2<Integer, Integer>> model = getRuntimeContext().getBroadcastVariable(broadcastModelKey);
