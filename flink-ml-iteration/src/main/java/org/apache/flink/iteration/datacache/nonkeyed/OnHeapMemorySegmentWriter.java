...
/** A class that writes cache data to on-heap memory. */
public class OnHeapMemorySegmentWriter<T> implements SegmentWriter<T> {
    ...
    /** The pool to allocate on-heap memory space from. */
    private final OnHeapMemoryPool onHeapMemoryPool;

    /** The cached data. */
    private final List<T> list = new ArrayList<>();

    public OnHeapMemorySegmentWriter(Path path, OnHeapMemoryPool onHeapMemoryPool) {
        this.path = path;
    ...
    public boolean addRecord(T record) throws IOException {
        ...
        if (!onHeapMemoryPool.acquireMemory(recordSize)) {
            return false;
        }
        list.add(record);
        return true;
