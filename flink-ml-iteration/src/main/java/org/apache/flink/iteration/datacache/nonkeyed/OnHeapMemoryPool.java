...
/** A class that manages bookkeeping for a fixed-size on-heap memory space. */
@Internal
public class OnHeapMemoryPool {
    ...
    /**
     * Gets or creates the {@link OnHeapMemoryPool} instance that corresponds to the given {@link
     * StreamTask} and registers it as a singleton object. There would be only one {@link
     * OnHeapMemoryPool} instance for each JVM (TaskManager).
     */
    public static OnHeapMemoryPool getOrCreate(StreamTask<?, ?> containingTask) {
        ...
        configuration = containingTask.getEnvironment().getTaskManagerInfo().getConfiguration();
    ...
    protected void finalize() throws Throwable {
        ...
        Preconditions.checkState(memoryUsed == 0);
    }
}
